...
const ViewOperator = () => {
  ...
  useEffect(() => {
    const fetchOperators = async () => {
      try {
        setLoading(true);
        setError(null);
        const result = await axios.get('https://frflf-backend.onrender.com/allop');
  ...
  const handleDisplay = async (e, id) => {
    ...
    try {
      ...
      const result = await axios(`https://frflf-backend.onrender.com/Operatorprofile/${id}`);
  ...
  const handleDelete = async (e, id) => {
    ...
    try {
      ...
      const response = await axios.delete(`https://frflf-backend.onrender.com/opdelete/${id}`);
    ...
    return (
      <div className="min-h-screen flex items-center justify-center mt-[100px]">
        <div className="text-center">
          ...
          <p className="mt-4 text-xl font-semibold text-gray-700">Loading operators...</p>
        </div>
      </div>
    );
  }
        ...
        <div className="flex flex-col gap-6 max-w-4xl mx-auto">
          {users.length > 0 ? (
            ...
            <div className="text-center mt-10">
              <h1 className="text-3xl font-bold text-gray-600">No Operators Found</h1>
            </div>
          )}
        </div>
      </div>
