        ...
        {loading ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-6">
            {images.map((file, index) => {
              ...
              const imageUrl = `https://frflf-backend.onrender.com/matched_faces/${name}/${file}`;
              return (
                <div
                  key={index}
                ...
                >
                  <img
                    ...
                    onClick={() => handleImageClick(criminalName, cameraName, date, time, imageUrl)}
                  />
                  <div className="p-3 bg-gray-100">
                    <p className="text-sm text-gray-700 font-medium">Evidence #{index + 1}</p>
                    <div className="p-4 bg-gray-100 space-y-1">
                      <p className="text-gray-800 text-md">
                        <span className="font-bold text-black">Name:</span> {criminalName}
                      </p>
                      <p className="text-gray-600 text-md">
                        <span className="font-bold text-black">Camera:</span> {cameraName}
                      </p>
                      <p className="text-gray-600 text-md">
                        <span className="font-bold text-black">Date:</span> {date}
                      </p>
                      <p className="text-gray-600 text-md">
                        <span className="font-bold text-black">Time:</span> {time.replace(/-/g, ':')}
                      </p>
                    </div>
                  </div>
                </div>
